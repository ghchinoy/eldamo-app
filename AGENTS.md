...
## Project Architecture & Engineering Lessons

### Native OS Menu (Wails v2)
- Custom menu bar configured in `main.go` using `menu.NewMenu()`.
- Top-level submenus: Application Menu ("Eldamo"), `menu.EditMenu()`, `menu.WindowMenu()`.
- Menu item clicks dispatch Go runtime events (`menu_open_about`, `menu_open_config`), which `api.ts` listens for to toggle Lit component modals.
- Version string source of truth is defined in `internal/app/app.go` (`AppVersion = "1.0.0"`) and exposed via Wails `GetAppVersion()`.

### Glaemscribe & Tengwar Transliteration
- **Charset Encoding**: Always use `tengwar_guni_*` (Glaemunicode Private-Use-Area) charsets for Glaemscribe transcription in this app. The old Dan Smith (`tengwar_ds_*`) charsets map glyphs onto ASCII characters (0x20-0x7E) which cause garbled text with the vendored web fonts.
- **Web Fonts**: Embedded `@font-face` definitions for Tengwar Annatar, Eldamar, and Parmaite live in `frontend/src/styles/tengwar-styles.ts`.
